...
func _check_oaks_lab_has_pc_interactable() -> void:
	...
	_check_equal(scene_source.contains("position = Vector2(112, 48)"), true, "PokemonPC marker is on the blue PC")
	...
	_check_equal(scene_source.contains("required_facing_direction = 1"), true, "PokemonPC requires facing up")
