...
func _check_ui_overlay_wrapper_exists() -> void:
	...
	_check_equal(source.contains("position_beside_rect_within(button.get_global_rect(), safe_bounds)"), true, "PC hover details stay beside their slot without covering navigation")
	...
	_check_equal(source.contains("func _preview_pc_slot_move"), true, "UIOverlay previews PC moves before the server response")
	...
	_check_equal(source.contains("pc_box_selector_button"), true, "UIOverlay uses one named PC box selector")
	_check_equal(source.contains("func _refresh_pc_box_navigation"), true, "UIOverlay refreshes PC box navigation")
	_check_equal(source.contains("func _apply_pc_box_selector_item_style"), true, "UIOverlay styles the active box inside the selector")
