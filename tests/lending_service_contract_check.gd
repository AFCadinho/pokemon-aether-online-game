...
func _init() -> void:
	...
	for contract in ["func create_loan", "func accept_loan", "func decline_loan", "func cancel_loan", "func request_return", "func decline_return", "func return_assets", "func attach_item", "func detach_item", "func load_notifications", "func acknowledge_notification", "func load_return_inbox", "func acknowledge_return"]:
	...
	_check(workspace_source.contains("_on_history_search_changed") and workspace_source.contains("_open_history_filter") and workspace_source.contains("history_period_filter := \"30\"") and workspace_source.contains("_history_date_is_valid"), "30-day loan history has search, preset periods, and a custom date filter")
	_check(source.contains("statuses=%s") and source.contains("normalized_search.uri_encode()") and source.contains("dateFrom=%s&dateTo=%s") and source.contains("days=%d"), "loan history search, status, and date filters are sent to the backend")
	...
	_check(workspace_source.contains("_asset_decline_return_button") and workspace_source.contains("decline_return(loan_id, asset_id)"), "borrowers can accept or decline each return request")
	_check(workspace_source.contains("return_requests_changed") and workspace_source.contains("open_return_requests") and overlay_source.contains("socials_loans_attention_badge"), "return requests drive Social and Loans attention and open the requested asset")
	...
	_check(workspace._history_date_is_valid("2026-08-24"), "history accepts an ISO calendar date")
	_check(not workspace._history_date_is_valid("2026-02-30"), "history rejects an impossible calendar date")
	...
	var requested_row := workspace._loan_asset_row({"assetId": "pokemon-request", "assetType": "pokemon", "status": "active", "returnRequestedAt": "2026-08-23T20:00:00+00:00", "snapshot": {"speciesId": "pikachu", "level": 20}}, true, "active", "pokemon-loan", 1, 1)
	_check("ui.lending.accept_return_request" in _button_texts(requested_row) and "ui.lending.decline_return_request" in _button_texts(requested_row), "a requested asset exposes accept-and-return and decline actions")
