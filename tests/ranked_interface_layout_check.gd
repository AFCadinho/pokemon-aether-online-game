...
func _init() -> void:
	...
	_check(source.contains('func _refresh_pvp_ranked_rewards_state() -> void:') and source.contains('queue.get("battlePointRewards", {})'), "Rewards availability follows the selected queue's authoritative server policy")
	_check(source.contains('"ui.pvp.rewards.status_active"') and source.contains('"ui.pvp.rewards.status_inactive"'), "Rewards distinguishes active and inactive server states")
