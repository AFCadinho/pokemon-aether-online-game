...
func _run() -> void:
	...
	_expect("_future_self_spawn_global_position = player.global_position + Vector2(fossil_side * 32.0, 0)" in controller_source, "the rescuer appears exactly one tile beside the player on the unchosen fossil side")
	_expect("player.global_position + Vector2(0, 32) if player != null" in controller_source, "the future starter appears one tile below the player")
