...
func _init() -> void:
	...
	_check(source.contains("button.mouse_default_cursor_shape = Control.CURSOR_CAN_DROP"), "Valid storage targets advertise that a Pokémon can be dropped")
	_check(source.contains("pc_box_title_label.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND"), "Clickable box title uses a pointer")
