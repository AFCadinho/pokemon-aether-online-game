...
func _init() -> void:
	...
	_check(not source.contains('search_scope_panel.name = "SearchScopeBadge"'), "Storage does not show a misleading all-boxes badge")
	_check(source.contains('pc_search_results_label.visible = false'), "Search result count stays hidden outside search mode")
	_check(source.contains('"secondary":'), "Storage distinguishes secondary actions from the primary selector")
	...
	var selector_selection_start := source.find("func _on_pc_selector_box_selected(index: int) -> void:")
	var selector_selection_end := source.find("\nfunc ", selector_selection_start + 1)
	var selector_selection_source := source.substr(selector_selection_start, selector_selection_end - selector_selection_start)
	_check(not selector_selection_source.contains("_close_pc_box_selector()"), "Changing boxes keeps the selector open")
	...
	_check(not source.contains("pc_slot_context_menu.popup_hide.connect(_clear_pc_slot_context)"), "Closing the context menu does not clear its action before selection")
