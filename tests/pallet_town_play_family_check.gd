...
func _run() -> void:
	...
	if child != null and pikachu != null:
		...
		_check(playmate_source.contains("func _after_base_npc_process()"), "Playmate synchronization uses BaseNPC's serialized lifecycle hook")
		...
		_check(not playmate_source.contains("func _physics_process(_delta: float)"), "Playmate does not compete with dialogue from a physics callback")
