...
func _init() -> void:
	...
	service.apply_snapshot({"tradeId":"t", "status":"completed", "revision":7, "lastEventSeq":7})
	_check(service.active_trade_id == "", "terminal snapshot immediately releases active discovery state")
	_check(str(service.active_trade_snapshot.get("status", "")) == "completed", "terminal snapshot remains available to the UI")
