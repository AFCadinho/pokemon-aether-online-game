...
func _run() -> void:
	...
	assert(catalog.size() == 14 and Registry.DATA.data.models.size() == 152)
	...
	# The original local 75 now contain 55 approved pairs and 20 normal-only holds.
	# This legacy portable pack still contains only its seven original pairs.
	assert(Registry.SCREENED.data.models.size() == 20)
