...
func _init() -> void:
	...
	_check(source.contains("money_title.text = \"MONEY OFFER\"") and source.contains("update_money_button.text = \"Update\""), "money controls use a structured offer header and explicit action")
