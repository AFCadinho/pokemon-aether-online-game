...
func _init() -> void:
	...
	_check(source.contains('"ui.trade.items.search"') and source.contains("func _filter_item_selector_rows"), "item selector supports localized scalable inventory search")
	...
	_check(source.contains('"ui.trade.offer.money"') and source.contains('"ui.trade.money.update"'), "money controls use a localized structured offer header and explicit action")
	_check(source.contains("money_controls") and source.contains("currency_prefix"), "money entry is grouped on its own input row")
