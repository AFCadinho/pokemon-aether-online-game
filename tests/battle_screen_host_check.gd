...
func _run() -> void:
	...
	for round_index in 3:
		...
		if round_index == 0:
			...
			host.add_child(chat_entry)
			chat_entry.grab_focus()
			host._input(escape)
			assert(not host.battle_settings_menu.visible, "Escape must leave active text input before opening Settings")
			chat_entry.release_focus()
			chat_entry.queue_free()
			battle.calc_drawer.show()
			host._input(escape)
			assert(not battle.calc_drawer.visible and not host.battle_settings_menu.visible, "Escape closes an active battle drawer before opening Settings")
			host._input(escape)
			assert(host.settings_overlay.visible and host.battle_settings_menu.visible, "Escape opens Settings over a dedicated battle")
			assert(not battle.is_processing_unhandled_input(), "Settings blocks battle shortcuts while open")
			host.battle_settings_menu.close()
			assert(not host.settings_overlay.visible and battle.is_processing_unhandled_input(), "Closing Settings restores battle input")
