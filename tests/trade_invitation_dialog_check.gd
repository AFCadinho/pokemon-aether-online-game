...
func _init() -> void:
	...
	dialog.hide()
	var duel_controller := Node.new()
	duel_controller.add_to_group("aether_clash_duel_controller")
	root.add_child(duel_controller)
	var duel_snapshot := incoming_snapshot.duplicate(true)
	duel_snapshot["tradeId"] = "trade-duel-blocked"
	dialog._on_trade_changed(duel_snapshot)
	_check(not dialog.visible, "Guild Duel maps suppress incoming Trade invitation popups")
	_check(system_overlay.messages.size() == 1, "suppressed Duel invitations do not emit disruptive messages")
	...
	_check(source.contains("aether_clash_exchange_surface"), "Guild Duel entry can immediately close an existing Trade surface")
