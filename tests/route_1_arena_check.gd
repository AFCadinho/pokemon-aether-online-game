...
func _run() -> void:
	...
	# Route 1 currently exposes grass encounters only; unexpected water contexts
	# keep the generic water fallback rather than inventing an inaccessible arena.
	assert(Resolver.resolve({"battle_kind": "wild", "map_id": "kanto_route_1", "encounter_type": "surf"}) == &"water")
	...
	for pass_data in pool.passes:
		...
		assert(water.position.y > _height(vertices, grid, Vector3(water.position.x, 0, water.position.z)) + 0.7)
