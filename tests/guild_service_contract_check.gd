...
func _init() -> void:
	...
	_check_contains(service_source, 'GUILD_HOME_ENDPOINT := "/game/guilds/me"', "browser guild home uses the shared route")
	_check_contains(service_source, 'GUILD_LOBBY_TELEPORT_ENDPOINT := "/game/guilds/me/lobby/teleport"', "browser Guild Lobby travel uses the shared route")
	...
	_check_contains(service_source, 'GUILD_HOME_ENDPOINT + "/leadership/%d"', "leadership transfer uses the dedicated member endpoint")
	_check_contains(service_source, "func disband_guild", "guild service exposes solo Guild disbanding")
	_check_contains(service_source, "HTTPClient.METHOD_DELETE", "Guild disbanding uses a destructive request")
