extends SceneTree
## Fresh-process A/B review. --terrain selects the approved Terrain3D baseline;
## default is the native-free prototype. --water selects the shared pond camera.
const Catalog = preload("res://scripts/battle/arenas/arena_catalog.gd")
const Art = preload("res://scripts/battle/arenas/forest_art_pack.gd")
const MeshArena = preload("res://scripts/battle/arenas/route_22_mesh_arena.gd")
...
func _run() -> void:
	...
	var terrain := "--terrain" in args
	var water := "--water" in args
	var arena_id := "route_22_water" if water else "route_22"
	var label := ("terrain" if terrain else "mesh") + ("-water" if water else "-land")
	...
	if terrain:
		...
		assert(Art.prepare(manifest).is_empty())
	...
	for response in [false, true]:
		...
		var arena: Node3D
		if terrain:
			arena = Catalog.build(arena_id, world, camera)
		else:
			var builder := MeshArena.new()
			builder.water_battle = water
			arena = builder.build(camera)
		world.add_child(arena)
		scenes.append(arena)
		build_ms.append((Time.get_ticks_usec() - pass_started) / 1000.0)
		_check(arena, terrain, water)
		await process_frame
	...
	if not terrain:
		assert(not ClassDB.class_exists("Terrain3D"), "No native module may load, including indirectly through art")
		assert(scenes[0].get_node("MeshTerrain").mesh == scenes[1].get_node("MeshTerrain").mesh)
		assert(scenes[0].get_node("Route22Scenery/SharedForestGrass").get_child(0).multimesh == scenes[1].get_node("Route22Scenery/SharedForestGrass").get_child(0).multimesh)
	...
	if water and not terrain and DisplayServer.get_name() != "headless":
		for i in 2:
...
func _check(arena: Node3D, terrain: bool, water: bool) -> void:
