...
func _init() -> void:
	...
	_check_contains(FileAccess.get_file_as_string("res://scripts/world/npcs/trainer_npc.gd"), "func _reload_trainer_progress", "visible trainers reload after a developer checkpoint")
	_check_contains(popup_script, 'story_chapter_select.name = "StoryChapterSelect"', "trainer progress groups checkpoints by story chapter")
	_check_contains(popup_script, '"chapter_id": "mt_moon"', "Mt. Moon checkpoints share a scalable chapter group")
	_check_contains(popup_script, 'badge_service.call("load_gym_badges")', "story checkpoints refresh badge ownership and level caps")
	...
	_check_contains(popup_script, '"mt_moon_warning"', "trainer progress exposes the Mt. Moon entrance checkpoint")
	_check_contains(popup_script, '"mt_moon_grunts"', "trainer progress exposes the Mt. Moon grunt checkpoint")
	_check_contains(popup_script, '"mt_moon_miguel"', "trainer progress exposes the Miguel checkpoint")
	_check_contains(popup_script, '"mt_moon_fossil"', "trainer progress exposes the fossil checkpoint")
	_check_contains(popup_script, '"mt_moon_rescue"', "trainer progress exposes the Mt. Moon rescue checkpoint")
	_check_contains(popup_script, '"chapter_id": "cerulean"', "Cerulean checkpoints use their own scalable chapter group")
	_check_contains(popup_script, '"cerulean_gym"', "trainer progress exposes the Cerulean Gym arrival checkpoint")
	_check_contains(popup_script, '"cerulean_nugget_bridge"', "trainer progress exposes the Nugget Bridge checkpoint")
	_check_contains(popup_script, '"help_bill"', "trainer progress exposes Bill's rescue checkpoint")
