...
func _init() -> void:
	...
	_check(battle_api.contains('"/account/pvp/training-ai/statistics"'), "web AI statistics use the shared endpoint")
	_check(battle_api.contains('"/account/pvp/training-ai/history/me"'), "web AI history uses the shared endpoint")
	...
	_check(not overlay.contains('_show_web_client_required("Ranked PvP")'), "Ranked is available in the browser")
