...
func _init() -> void:
	...
	_check(source.count("_prepare_confirmation_dialog_focus(staff_teleport_send_confirmation)") >= 2, "Staff teleport confirmation enables focus before setup and display")
	_check(source.contains("confirm_button.set_focus_behavior_recursive(Control.FOCUS_BEHAVIOR_ENABLED)") and source.contains("cancel_button.set_focus_behavior_recursive(Control.FOCUS_BEHAVIOR_ENABLED)") and source.contains("confirm_button.focus_mode = Control.FOCUS_ALL") and source.contains("cancel_button.focus_mode = Control.FOCUS_ALL"), "Confirmation dialogs explicitly allow keyboard and controller focus")
	...
	_check(source.count("_hide_staff_teleport_popup()") >= 3, "Successful staff teleports close the destination window")
