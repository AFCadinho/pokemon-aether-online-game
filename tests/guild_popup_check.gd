...
func _init() -> void:
	...
	_check_contains(popup_source, "MEMBER ROSTER", "guild dashboard renders its member roster")
	...
	_check_contains(popup_source, 'tooltip_text = "Edit Guild emblem"', "editable guild emblem explains its action")
	...
	_check_contains(popup_source, "INVITE A TRAINER", "guild leaders and officers can invite trainers")
