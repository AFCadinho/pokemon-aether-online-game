...
func _init() -> void:
	...
	_check(scene_source.contains("custom_minimum_size = Vector2(332, 82)"), "location card keeps its compact status layout")
	...
	_check(
		not scene_source.contains('[node name="AetherClashChampionLabel"')
		and not script_source.contains("GuildService.load_aether_clash_champion()"),
		"Guild Duel results are not presented as a reigning Guild in the shared lobby"
	)
