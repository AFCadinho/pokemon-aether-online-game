...
func _init() -> void:
	...
	workspace.status_label = Label.new()
	workspace.status_panel = PanelContainer.new()
	workspace._show_error("Trade cap error", true)
	workspace._set_status("")
	_check(workspace.status_label.text == "Trade cap error", "offer errors survive realtime status redraws")
	workspace._clear_persistent_offer_error()
	workspace._set_status("Normal status")
	_check(workspace.status_label.text == "Normal status", "a new offer attempt clears the persistent error")
	workspace.status_label.free()
	workspace.status_panel.free()
	workspace.status_label = null
	workspace.status_panel = null
	...
	_check(source.contains("status_panel.visible = visible_message.strip_edges() != \"\""), "empty status chrome does not waste vertical space")
