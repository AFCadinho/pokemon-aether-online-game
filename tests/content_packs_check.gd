...
func run() -> void:
	...
	check(store.save_selected_by_category({"cries": "valid", "battle_sprites": "valid", "followers": "valid"}) == OK, "per-category selections saved")
	...
	check(frames != null and frames.get_frame_count("idle") == 2, "selected sprite pack resolves")
	if frames != null:
	...
	check(Runtime.cry("Pikachu") != null, "selected cry pack loads")
	check(Runtime.cry("Pikachu-Mega") != null, "selected cry pack falls back through the base cry key")
	check(Runtime.cry("missing") == null, "missing cry preserves game fallback")
	check(Runtime.get_pokemon_sprite_roots().size() == 1, "selected Gen 5 collection root loads")
	check(Followers.get_sprite_frames("Pikachu", false) != null, "follower service consumes pack texture")
	check(store.save_selected_by_category({}) == OK, "clear all category selections")
