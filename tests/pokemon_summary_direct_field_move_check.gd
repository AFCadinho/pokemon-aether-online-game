...
func _init() -> void:
	...
	_check_true(service_source.contains('"rain-dance": {'), "Rain Dance is exposed as a direct overworld move")
	_check_true(service_source.contains('"snowscape": {'), "Snowscape is exposed as a direct overworld move")
	_check_true(service_source.contains('"sunny-day": {'), "Sunny Day is exposed as a direct overworld move")
	_check_true(service_source.contains('const WEATHER_ACTION_ENDPOINT := "/world/weather/action"'), "weather moves use the server-authoritative action endpoint")
	_check_true(ResourceLoader.exists("res://assets/items/icons/field_move_charms/SUNNYDAYCHARM.png"), "Sunny Day Charm has a dedicated cropped icon")
