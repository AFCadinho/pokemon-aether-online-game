...
func _init() -> void:
	...
	_check(source.contains('"ui.staff.impersonate.subtitle"'), "Header explains the temporary session")
	_check(source.contains('"ui.staff.impersonate.security_title"'), "Sensitive action has a prominent security notice")
	...
	_check(source.contains('"ui.staff.impersonate.confirm"'), "Primary action clearly describes the secure session")
	...
	_check(source.contains('"ui.staff.impersonate.verifying"') and source.contains('"ui.staff.impersonate.verifying_status"'), "Authentication progress is visible")
	...
	_check(
		source.find('world.call("prepare_for_account_switch")')
		< source.find("await AuthService.impersonate_with_token(token)"),
		"Impersonation blocks the previous account runtime before exchanging tokens"
	)
	_check(
		source.contains("get_tree().change_scene_to_file(LOADING_SCENE_PATH)"),
		"Impersonation rebuilds the world through the loading screen"
	)
