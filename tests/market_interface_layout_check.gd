...
func _init() -> void:
	...
	_check_true(text.contains("Search the catalog..."), "market catalog is searchable")
	...
	_check_true(text.contains("Unit price"), "market distinguishes unit price")
	_check_true(text.contains("market_total_price_label"), "market shows the purchase total")
	_check_true(text.contains("Purchases are delivered directly to your Bag."), "market explains item delivery")
