...
func _init() -> void:
	...
	_check_true(text.contains("market_sort_select"), "market catalog has neutral sorting controls")
	_check_true(text.contains("func _sort_market_items"), "market sorts catalog items without role categories")
	_check_true(text.contains('"id": str(market.get("id", "standard_pokemart"))'), "named markets retain their purchase id")
