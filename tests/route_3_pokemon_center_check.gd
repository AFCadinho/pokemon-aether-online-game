...
func _run() -> void:
	...
	_check(mt_moon_lock.contains('position = Vector2(2640, 656)'), "Mt. Moon lock is aligned with the cave mouth")
	_check(mt_moon_lock.contains('display_name = "Mt. Moon"'), "Mt. Moon lock identifies the closed area")
	_check(mt_moon_lock.contains('blocked_tile_offset = Vector2i(-1, 0)'), "Mt. Moon lock starts at the cave mouth's left tile")
	_check(mt_moon_lock.contains('blocked_tile_footprint = Vector2i(3, 1)'), "Mt. Moon lock covers all three entrance tiles")
...
func _check_lock_footprint() -> void:
	var lock_scene := load("res://scenes/world/interactables/locked_door_interactable.tscn") as PackedScene
	var lock := lock_scene.instantiate()
	lock.position = Vector2(2640, 656)
	lock.set("blocked_tile_offset", Vector2i(-1, 0))
	lock.set("blocked_tile_footprint", Vector2i(3, 1))
	root.add_child(lock)
	_check(lock.call("blocks_world_position", Vector2(2608, 656)), "wide lock blocks the entrance's left tile")
	_check(lock.call("blocks_world_position", Vector2(2640, 656)), "wide lock blocks the entrance's middle tile")
	_check(lock.call("blocks_world_position", Vector2(2672, 656)), "wide lock blocks the entrance's right tile")
	_check(not lock.call("blocks_world_position", Vector2(2704, 656)), "wide lock leaves the next tile open")
	lock.queue_free()
