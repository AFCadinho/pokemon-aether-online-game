...
func _run() -> void:
	...
	_check(mt_moon_lock.contains('blocked_tile_footprint = Vector2i(2, 1)'), "Mt. Moon lock covers both entrance tiles")
...
func _check_lock_footprint() -> void:
	...
	lock.set("blocked_tile_footprint", Vector2i(2, 1))
	root.add_child(lock)
	_check(lock.call("blocks_world_position", Vector2(2608, 656)), "wide lock blocks the entrance's left tile")
	_check(lock.call("blocks_world_position", Vector2(2640, 656)), "wide lock blocks the entrance's right tile")
	_check(not lock.call("blocks_world_position", Vector2(2672, 656)), "wide lock leaves the next tile open")
	lock.queue_free()
