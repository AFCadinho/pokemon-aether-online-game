...
func _init() -> void:
	...
	_check(source.contains('quantity_label.text = LocalizationManager.text("ui.bag.key_marker") if bool(item.get("permanent", false)) else "x%s"'), "Bag distinguishes permanent items from stack quantities with a localized marker")
