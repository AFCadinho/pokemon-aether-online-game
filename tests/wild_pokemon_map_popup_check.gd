...
func _init() -> void:
	...
	_check_contains(script_source, "func _weekly_availability_days", "popup and Pokédex share weekly availability metadata parsing")
	_check_contains(script_source, "_weekly_availability_is_active_today(available_days)", "popup hides rotation entries outside the current world weekday")
	_check_contains(script_source, 'rotation_label.name = "WildPokemonRotationStatus"', "popup identifies an active daily rotation")
	_check_contains(script_source, '"ui.wild.rotation_today"', "popup describes the current rotation day")
	_check_contains(script_source, "displayed_world_weekday_id != weekday_id", "an open popup refreshes when the world weekday changes")
