...
func _init() -> void:
	...
	_check(source.contains('if not context_more_actions_expanded:'), "Context card keeps secondary actions collapsed by default")
	_check(source.contains('context_actions.add_child(_context_section_label("MORE ACTIONS"))'), "Expanded actions retain a clear group label")
