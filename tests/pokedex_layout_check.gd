...
func _init() -> void:
	...
	_check(source.contains('not pokedex_selected_species.has("wildDropCatalogId")') and source.contains("_refresh_selected_pokedex_drop_contract"), "Opening Drops refreshes detail data cached before the drop catalog was available")
	_check(source.contains('not (cached_species as Dictionary).has("wildDropCatalogId")') and source.contains("get_species_detail(normalized_species_id, true)"), "Selecting a species replaces stale pre-drop-catalog detail data")
