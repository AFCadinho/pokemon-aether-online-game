...
func _init() -> void:
	...
	_check(source.contains("func _on_item_dex_results_scrolled") and source.contains("func _load_more_item_dex_results"), "Item Dex loads additional items near the bottom of the list")
	_check(source.contains("item_dex_results_loaded_offset") and source.contains("ITEM_DEX_PAGE_SIZE := 10"), "Item Dex pagination requests stable batches of ten")
	var inventory_service_source := FileAccess.get_file_as_string("res://scripts/services/inventory_service.gd")
	_check(inventory_service_source.contains('"&limit=%d&offset=%d"'), "Item Dex item searches send their page size and offset")
