...
func _check_npc_entry_hazard_faint_requests_another_replacement() -> void:
	...
	var terminal_guard := function_source.find("if battle_finished or battle_state.is_battle_ended():", function_source.find("await _hold_opponent_response_message()"))
	var force_switch_check := function_source.find("var response_requires_switch :=")
	...
	_check_equal(
		...
		and terminal_guard >= 0
		and terminal_guard < force_switch_check
		and function_source.count("action_flow.submit_npc_choice") == 1,
		true,
		"an NPC replacement continues only while the entry-hazard result remains nonterminal"
	)
	_check_equal(
