...
func _check_scene_structure() -> void:
	...
	var switch_label_start := scene_source.find("[node name=\"SwitchPartyLabel\"")
	var switch_label_end := scene_source.find("\n\n", switch_label_start)
	var switch_label_block := scene_source.substr(switch_label_start, switch_label_end - switch_label_start)
	var switch_divider_start := scene_source.find("[node name=\"SwitchDivider\"")
	var actions_dock_start := scene_source.find("[node name=\"ActionsDock\"")
	_check_true(player_stage_grid_start < switch_divider_start and switch_divider_start < actions_dock_start, "switch divider is ordered between battlefield and party dock")
	_check_contains(scene_source.substr(switch_divider_start, switch_label_start - switch_divider_start), "custom_minimum_size = Vector2(0, 1)", "switch divider preserves party and battlefield height")
	_check_contains(switch_label_block, 'parent="HBoxContainer/CenterColumn/SwitchDivider"', "switch title lives between the battlefield and party row")
	_check_contains(switch_label_block, "layout_mode = 1", "switch title uses absolute positioning")
	...
	_check_contains(FileAccess.get_file_as_string(BATTLE_SCRIPT_PATH), "anchor_stat_stage_panel_below", "battle anchors enemy stat badges below the enemy HUD")
	...
	_check_contains(calc_drawer_block, "offset_right = 620.0", "Calc drawer has a stable editor fallback size")
	...
	_check_contains(battle_script_source, "CALC_DRAWER_FIELD_WIDTH_RATIO := 0.55", "Calc drawer covers only the player field section")
	...
	_check_contains(battle_script_source, "frame_size.x * CALC_DRAWER_FIELD_WIDTH_RATIO", "Calc drawer keeps the opponent battlefield side accessible")
	...
	_check_contains(scene_source, "[node name=\"UtilityActions\" type=\"HBoxContainer\" parent=\"HBoxContainer/CenterColumn/BattleFrame/MarginContainer/BattleStageViewport/BattleStage\"", "battlefield has a compact utility header")
	_check_contains(scene_source, "[node name=\"BagButton\" type=\"Button\" parent=\"HBoxContainer/CenterColumn/BattleFrame/MarginContainer/BattleStageViewport/BattleStage/UtilityActions\"", "Bag sits above the move grid")
	_check_contains(scene_source, "[node name=\"RunButton\" type=\"Button\" parent=\"HBoxContainer/CenterColumn/BattleFrame/MarginContainer/BattleStageViewport/BattleStage/UtilityActions\"", "Run or Forfeit sits above the move grid")
	_check_contains(scene_source, "custom_minimum_size = Vector2(40, 38)", "battlefield utility actions remain compact icon buttons")
	_check_contains(scene_source, "res://assets/battles/ui/bag_white.svg", "Bag uses a white utility icon")
	_check_contains(scene_source, "res://assets/battles/ui/exit_white.svg", "Run or Forfeit uses a white exit icon")
...
func _check_battle_selection_policy_contract() -> void:
	...
	_check_contains(source, "switch_party_label.visible = not is_calc_mode and not is_bag_view and not show_pvp_switch_confirmation", "floating switch title yields to the accepted PvP switch confirmation")
	...
	_check_contains(source, '"%s will switch in, replacing %s."', "switch confirmation names both incoming and replaced Pokemon")
