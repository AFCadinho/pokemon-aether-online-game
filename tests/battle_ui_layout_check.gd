...
func _check_scene_structure() -> void:
	...
	_check_contains(player_hud_block, "offset_top = 170.0", "player HP HUD keeps its compact vertical bounds")
	_check_contains(player_hud_block, "offset_bottom = 240.0", "player HP HUD keeps its compact vertical bounds")
	...
	_check_contains(enemy_hud_block, "offset_top = 70.0", "enemy HP HUD keeps its compact vertical bounds")
	_check_contains(enemy_hud_block, "offset_bottom = 140.0", "enemy HP HUD keeps its compact vertical bounds")
	...
	for effects_panel_name: String in ["SideFieldEffectsPanel", "SideFieldEffectsPanel2"]:
		...
		_check_contains(effects_block, "z_index = 110", "%s renders above battle-stage UI" % effects_panel_name)
	_check_contains(scene_source, "offset_left = 92.0", "player field indicators sit against the player party preview")
	_check_contains(scene_source, "offset_top = 252.0", "player field indicators sit below the player HP HUD")
	_check_contains(scene_source, "offset_right = -83.0", "opponent field indicators sit against the opponent party preview")
	_check_contains(scene_source, "offset_top = 152.0", "opponent field indicators sit below the opponent HP HUD")
...
func _check_battle_selection_policy_contract() -> void:
	...
	_check_contains(source, "player_side_effects_panel.visible = false", "calculator hides the player battlefield side-condition label")
	_check_contains(source, "enemy_side_effects_panel.visible = false", "calculator hides the opponent battlefield side-condition label")
	_check_contains(source, "_update_side_condition_ui(_get_display_field_effects())", "battle view restores side-condition labels after closing Calc")
