...
func _check_scene_structure() -> void:
	...
	_check_contains(scene_source, "[node name=\"SpectatorActionPanel\" type=\"HBoxContainer\" parent=\"HBoxContainer/CenterColumn/ActionsDock", "spectator controls replace the interactive party row")
	...
	_check_contains(scene_source, "text = \"Bag\"", "Bag is rendered as a text action")
	...
	_check_contains(scene_source, "offset_left = 82.0", "player field indicators sit inside the player icon rail")
	_check_contains(scene_source, "offset_right = -82.0", "opponent field indicators sit inside the opponent icon rail")
...
func _check_battle_selection_policy_contract() -> void:
	...
	_check_contains(source, "action_buttons.set_action_visible(\"bag\", battle_type == BattleType.WILD)", "Bag is visible only in wild battles")
