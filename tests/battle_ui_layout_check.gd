...
func _check_scene_structure() -> void:
	...
	_check_contains(enemy_hud_block, "offset_top = 90.0", "enemy HP HUD clears the right-side timer")
	_check_contains(enemy_hud_block, "offset_bottom = 172.0", "enemy HP HUD keeps its pill dimensions after moving down")
...
func _check_battle_selection_policy_contract() -> void:
	...
	_check_contains(source, "switch_party_label.visible = not is_calc_mode and not is_bag_view", "switch title hides for drawer-focused modes")
