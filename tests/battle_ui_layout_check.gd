...
func _check_scene_structure() -> void:
	...
	_check_contains(scene_source, "custom_minimum_size = Vector2(72, 38)", "Run or Forfeit has room for a readable text label")
	_check_contains(scene_source, "res://assets/battles/ui/bag_white.svg", "Bag uses a white utility icon")
	_check_contains(scene_source, "text = \"Run\"", "wild battles display a Run action instead of an exit icon")
	...
	_check_contains(party_slot_source, "_set_color(PARTY_BACKGROUND, PARTY_BORDER, false)", "party slots use a neutral base instead of Pokemon type colors")
...
func _check_battle_selection_policy_contract() -> void:
	...
	_check_contains(action_choices_source, "if action == \"bag\"", "only the Bag remains an icon-only utility action")
	_check_contains(action_choices_source, "button.text = label", "Run or Forfeit is rendered as a button label")
