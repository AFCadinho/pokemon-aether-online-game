...
func _check_scene_structure() -> void:
	...
	_check_contains(scene_source, "[node name=\"CalcTimerDock\" parent=\"BattleDrawerLayer\" instance=ExtResource(\"9_pyfix\")]", "Calc has a dedicated timer dock above the battlefield")
	...
	_check_contains(battle_script_source, "enemy_hud_panel.get_global_rect().position", "Calc drawer measures the visible opponent HP HUD boundary")
	_check_contains(battle_script_source, "drawer_width = minf(drawer_width, opponent_safe_width)", "Calc drawer never covers the opponent HP HUD")
	...
	_check_contains(battle_script_source, "CALC_TIMER_DOCK_TWO_PANEL_MIN_WIDTH", "Calc timer dock adapts its secondary countdown to available width")
