...
func _check_scene_structure() -> void:
	...
	_check_contains(pokemon_info_hud_source, "[node name=\"ExpTextLabel\" type=\"Label\"", "battle EXP row has an aligned label")
	...
	_check_contains(pokemon_info_hud_source, "custom_minimum_size = Vector2(0, 18)", "battle HP and EXP bars share a full-height presentation")
	...
	_check_contains(effects_script_source, '"stealthrock": "battle.field.short.stealth_rock"', "Stealth Rock uses a localized compact label")
