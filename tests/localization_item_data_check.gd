...
func _check_catalogs() -> void:
	...
	for locale: String in GENERATED_CATALOG_PATHS:
		...
		_check(catalog.size() == 1442, "generated %s item catalog covers the complete source index" % locale)
	...
	for locale: String in GENERATED_CATALOG_PATHS:
		...
		_check(
			(item_localization.call("get_catalog", locale) as Dictionary).size() == 1443,
