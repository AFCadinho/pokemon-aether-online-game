...
func _init() -> void:
	...
	_check(script_source.contains("func _setup_language_chat_ui()"), "official language chats use a dedicated main tab")
	...
	_check(script_source.contains("visibility_toggle.toggled.connect(_on_language_chat_toggled.bind(channel))"), "official language chats can be added and removed")
	...
	_check(settings_source.contains('LANGUAGE_CHAT_ZH := "language_zh"') and settings_source.contains('LANGUAGE_CHAT_PT := "language_pt"'), "settings expose Chinese and Portuguese official chats")
	...
	_check(script_source.contains("if active_chat_tab in LANGUAGE_CHAT_CHANNELS:") and script_source.contains("return category == active_chat_tab"), "the Languages tab shows only its selected official channel")
