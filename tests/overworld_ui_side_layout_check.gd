...
func _init() -> void:
	...
	_check(global_buffs_block.contains("custom_minimum_size = Vector2(289, 50)"), "global buffs use a compact six-icon tray")
	...
	_check(scene_source.count('parent="Control/GlobalBuffsPanel/MarginContainer/Row/BuffSlots"') == 6, "global buff tray exposes five community goals and Global Heal")
	...
	_check(script_source.contains("func _setup_chat_tab_settings_ui()"), "language chat settings are available without replacing the familiar chat layout")
	_check(script_source.contains("func _setup_language_chat_ui()"), "community language chats use a dedicated main tab")
	_check(script_source.contains('language_chat_empty_state.name = "LanguageChatEmptyState"'), "Languages begins with an actionable empty state")
	_check(script_source.contains("visibility_toggle.toggled.connect(_on_language_chat_toggled.bind(channel))"), "community language chats can be added and removed")
	_check(not script_source.contains("_on_chat_tab_move_pressed"), "main chat tabs keep a fixed order")
	...
	_check(settings_source.contains('"enabled_language_chats": enabled_language_chats'), "selected language chats are persisted with local settings")
	_check(settings_source.contains("func reset_enabled_language_chats()"), "language chat settings can be cleared")
	_check(settings_source.contains("var enabled_language_chats: Array[String] = []"), "new players start without optional language chats")
	...
	_check(script_source.count('"current": 0') == 5, "global buffs start with no community funding")
	_check(script_source.count('"state": "funding"') == 5 and not script_source.contains('"state": "active"'), "global buffs start inactive")
	...
	_check(script_source.contains('["global_exp", "global_skill_exp", "global_ev", "global_shiny", "global_rare_encounter"]'), "all fundable global boosts accept community contributions")
	_check(script_source.contains('"id": "global_skill_exp"'), "global buff data includes double skill experience")
	...
	_check(script_source.contains("PlayerWalletService.contribute_to_global_skill_exp_boost"), "global Skill EXP contributions use the authoritative wallet service")
	...
	_check(script_source.contains("_load_global_skill_exp_boost.call_deferred()"), "Skill EXP boost state loads from the server")
	...
	_check(script_source.contains('_load_global_boost_state.call_deferred("global_skill_exp", true)'), "global Skill EXP contribution events refresh authoritative boost state and activation cards")
