...
func _init() -> void:
	...
	_check(service.contains("func move_slot("), "hotbar-to-hotbar persistence")
	_check(ui.contains("_hotbar_inventory_quantity"), "hotbar inventory quantities")
	_check(ui.contains('LocalizationManager.text("ui.bag.key_marker")'), "localized permanent Key Item marker")
	_check(ui.contains('LocalizationManager.text("ui.hotbar.empty_tooltip"'), "localized empty-slot instructions")
	_check(ui.contains("_refresh_bag_localized_ui()"), "hotbar refreshes after a live locale change")
	_check(ui.contains("_on_hotbar_drop_highlight_changed"), "drop target highlight")
	...
	_check(ui.contains('PlayerHotbarService.assign(slot_index, "field_move"'), "Summary moves persist through the generic hotbar endpoint")
	_check(ui.contains("_activate_hotbar_field_move"), "field move hotbar entries activate through the shared field move service")
	_check(ui.contains('entry_id = "charm:%s"'), "field move Charms preserve their source in hotbar bindings")
