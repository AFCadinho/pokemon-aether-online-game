...
var pvp_active_queue_id := "ranked_queue_v1"
var pvp_active_format_key := PVP_RANKED_DEFAULT_FORMAT_KEY
var pvp_active_format_name := PVP_RANKED_DEFAULT_FORMAT_NAME
...
var staff_teleport_destination_search: LineEdit
...
func _current_user_role_ids() -> Array[String]:
	var roles_value: Variant = AuthService.current_user.get("roles", [])
	if not roles_value is Array:
		return []
	var roles: Array = roles_value as Array
...
func _setup_pvp_room_popup() -> void:
	...
	live_tab.add_child(live_scroll)

	pvp_live_list = VBoxContainer.new()
	pvp_live_list.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	pvp_live_list.add_theme_constant_override("separation", 6)
...
func _render_pvp_team_preview() -> void:
	if pvp_team_preview_grid == null:
		return
	for child in pvp_team_preview_grid.get_children():
		child.queue_free()
...
func _setup_staff_impersonation_tools() -> void:
	...
	staff_teleport_self_section.add_child(staff_teleport_destination_search)
	...
	staff_teleport_self_section.add_child(staff_teleport_spawn_search)

	staff_teleport_destination_results = ItemList.new()
	...
	staff_teleport_player_results = ItemList.new()
	...
	staff_teleport_send_section.add_child(_create_staff_teleport_section_title("Safe destination"))
	...
	staff_teleport_send_section.add_child(staff_teleport_send_map_search)
	...
	staff_teleport_send_section.add_child(staff_teleport_send_spawn_search)

	staff_teleport_send_destination_results = ItemList.new()
	...
	_apply_line_edit_style(staff_teleport_send_spawn_search)
...
func _bag_item_by_id(item_id: String) -> Dictionary:
	var normalized_id := _normalize_item_id(item_id)
	for item: Dictionary in bag_inventory_items:
		if _normalize_item_id(str(item.get("id", ""))) == normalized_id:
...
func _refresh_bag_inventory_for_machine_selection() -> void:
	while bag_inventory_loading:
...
func _load_staff_teleport_online_players_if_needed(force := false) -> void:
	...
	staff_teleport_online_players = players_value if players_value is Array else []
...
func _rebuild_staff_teleport_map_options() -> void:
	if staff_teleport_destination_results == null:
		return

	var selected_destination := _get_selected_staff_teleport_destination()
	var selected_map_id := str(selected_destination.get("map", {}).get("id", "")).strip_edges()
...
func _on_staff_teleport_spawn_search_changed(_text: String) -> void:
	_rebuild_staff_teleport_map_options()
...
func _update_staff_teleport_selected_destination_label() -> void:
	if staff_teleport_selected_destination_label == null:
		return
	var destination := _get_selected_staff_teleport_destination()
	var map_entry: Dictionary = _staff_dictionary_from_variant(destination.get("map", {}))
	var point_entry: Dictionary = _staff_dictionary_from_variant(destination.get("point", {}))
...
func _rebuild_staff_teleport_send_map_options() -> void:
	if staff_teleport_send_destination_results == null:
		return

	var selected_destination := _get_selected_staff_teleport_send_destination()
	var selected_map: Dictionary = _staff_dictionary_from_variant(selected_destination.get("map", {}))
...
func _on_staff_teleport_send_destination_search_changed(_text: String) -> void:
	_rebuild_staff_teleport_send_map_options()
...
func _rebuild_staff_teleport_player_options() -> void:
	if staff_teleport_player_results == null:
		return
	var selected_player := _get_selected_staff_teleport_player()
	var selected_player_id := int(selected_player.get("targetPlayerId", 0))
	staff_teleport_player_results.clear()
	staff_teleport_filtered_players = []
	var query := ""
	if staff_teleport_player_search_input != null:
		query = staff_teleport_player_search_input.text.strip_edges().to_lower()
	for player_value: Variant in staff_teleport_online_players:
		var player_entry: Dictionary = _staff_dictionary_from_variant(player_value)
		var username := str(player_entry.get("username", ""))
		var display_name := str(player_entry.get("displayName", username))
		var haystack := "%s %s" % [username.to_lower(), display_name.to_lower()]
		if query != "" and not haystack.contains(query):
			continue
...
func _on_staff_teleport_to_player_pressed() -> void:
	if not _can_teleport_to_player():
		_add_chat_message("You do not have permission to teleport to players.")
		return
	if staff_teleport_in_flight:
		return
...
func _get_selected_staff_teleport_map() -> Dictionary:
	var destination := _get_selected_staff_teleport_destination()
	return _staff_dictionary_from_variant(destination.get("map", {}))


func _get_selected_staff_teleport_point(selected_map: Dictionary) -> Dictionary:
	var destination := _get_selected_staff_teleport_destination()
	return _staff_dictionary_from_variant(destination.get("point", {}))


func _get_selected_staff_teleport_destination() -> Dictionary:
	if staff_teleport_destination_results == null:
		return {}
	var selected_items := staff_teleport_destination_results.get_selected_items()
...
func _get_selected_staff_teleport_send_destination() -> Dictionary:
	if staff_teleport_send_destination_results == null:
		return {}
	var selected_items := staff_teleport_send_destination_results.get_selected_items()
	if selected_items.is_empty():
		return {}
	var selected_index := int(selected_items[0])
...
func _get_selected_staff_teleport_player() -> Dictionary:
	if staff_teleport_player_results == null:
		return {}
	var selected_items := staff_teleport_player_results.get_selected_items()
	if selected_items.is_empty():
		return {}
	var selected_index := int(selected_items[0])
...
func _on_pvp_room_panel_gui_input(event: InputEvent) -> void:
	if pvp_room_popup == null:
		return
	if not (event is InputEventMouseButton):
		return
	...
	if mouse_event.button_index != MOUSE_BUTTON_LEFT:
		return
	if mouse_event.position.y > 56.0:
		return
	if mouse_event.pressed:
		pvp_room_dragging = true
		pvp_room_drag_offset = mouse_event.global_position - pvp_room_popup.global_position
...
func _refresh_pvp_team_validator() -> void:
	...
	if _is_selected_pvp_queue_ranked():
		...
		var server_is_authoritative := server_state in [
		...
		PvpRankedTeamValidation.STATE_INVALID:
			...
			for issue: Dictionary in server_issues:
				pvp_team_validator_list.add_child(_create_pvp_validator_row(
					...
					_pvp_ranked_validation_issue_message(issue),
					Color("#ff7979")
				))
...
func _pvp_validation_issue_value(issue: Dictionary) -> String:
...
func _pvp_party_slot_display_name(slot: int) -> String:
	if slot <= 0 or slot > PlayerSave.party.size():
		return ""
	var pokemon: Pokemon = PlayerSave.party[slot - 1]
	if pokemon == null:
		return ""
	return _format_pvp_species_display_name(pokemon.species)

func _format_pvp_species_display_name(species: String) -> String:
...
func _refresh_pvp_queue_list() -> void:
	...
	for item: Variant in queues_value as Array:
		...
		if str(queue.get("status", "")).strip_edges().to_lower() != "active":
			continue
		if str(queue.get("mode", "")).strip_edges().to_lower() != "ranked":
			continue
...
func _populate_pvp_queue_select(queues: Array[Dictionary]) -> void:
	...
	for queue: Dictionary in pvp_available_queues:
		...
		if queue_mode.to_lower() == "ranked":
			label = _pvp_queue_format_name(queue)
		elif queue_mode != "":
	...
	if pvp_queue_select.item_count == 0:
		pvp_queue_select.add_item(PVP_RANKED_DEFAULT_FORMAT_NAME)
...
func _pvp_queue_format_key(queue: Dictionary) -> String:
	var format_key := str(queue.get("formatKey", "")).strip_edges()
	if format_key == "":
		format_key = str(queue.get("formatId", "")).strip_edges()
	return format_key if format_key != "" else PVP_RANKED_DEFAULT_FORMAT_KEY

func _pvp_queue_format_name(queue: Dictionary) -> String:
	var format_name := str(queue.get("formatName", "")).strip_edges()
...
func _refresh_pvp_banlists(force: bool = false) -> void:
	...
	_render_pvp_banlists()
	var request := _create_pvp_request_node()
	var response: Dictionary = await BattleApiClient.get_pvp_ranked_banlists(request, pvp_active_format_key)
	request.queue_free()
	pvp_banlists_in_flight = false
...
func _render_pvp_banlist_metadata() -> void:
	...
	pvp_bans_metadata_list.add_child(_create_pvp_banlist_metadata_panel(metadata))

func _create_pvp_banlist_metadata_panel(metadata: Dictionary) -> Control:
	var panel := PanelContainer.new()
	panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	panel.add_theme_stylebox_override("panel", _make_panel_style(Color("#111926bb"), Color("#506982aa"), 4, 1))
...
func _create_pvp_banlist_category_panel(category: String) -> Control:
	if not pvp_banlist_category_open.has(category):
		pvp_banlist_category_open[category] = false
	var panel := PanelContainer.new()
	panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	...
	header_button.pressed.connect(_on_pvp_banlist_category_toggled.bind(category))
	_apply_button_style(header_button)
	layout.add_child(header_button)
	...
	items_list.add_theme_constant_override("separation", 5)
	layout.add_child(items_list)
	search_input.text_changed.connect(_on_pvp_banlist_search_changed.bind(category, items_list))
	_populate_pvp_banlist_category_items(category, items_list)
	return panel

func _populate_pvp_banlist_category_items(category: String, items_list: VBoxContainer) -> void:
	...
	for ban: Dictionary in bans:
		if not _pvp_ban_matches_search(ban, query):
			continue
		matches += 1
		var label := Label.new()
...
func _on_pvp_banlist_search_changed(text: String, category: String, items_list: VBoxContainer) -> void:
	pvp_banlist_category_search[category] = text
	_populate_pvp_banlist_category_items(category, items_list)
...
func _pvp_banlist_updated_label(value: String) -> String:
	...
	if dot_index >= 0:
		without_zone = without_zone.substr(0, dot_index)
	var parts := without_zone.split("T")
	if parts.size() != 2:
		return without_zone.replace("T", " ") + " UTC"
	var date_parts := String(parts[0]).split("-")
	...
	var month_label: String = month_names[month - 1] if month >= 1 and month <= 12 else String(parts[0])
...
func _on_pvp_leaderboard_scope_selected(index: int) -> void:
	if pvp_leaderboard_scope_select == null:
		return
	if index < 0 or index >= pvp_leaderboard_scope_select.item_count:
		return
	var scope := str(pvp_leaderboard_scope_select.get_item_metadata(index)).strip_edges()
	if scope == "":
		return
