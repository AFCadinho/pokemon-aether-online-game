...
const DRAG_CURSOR_PATH := "res://assets/ui/cursors/aether_drag.png"
const CAN_DROP_CURSOR_PATH := "res://assets/ui/cursors/aether_can_drop.png"
const FORBIDDEN_CURSOR_PATH := "res://assets/ui/cursors/aether_forbidden.png"
const ARROW_HOTSPOT := Vector2(1, 1)
const POINTER_HOTSPOT := Vector2(1, 1)
const DRAG_HOTSPOT := Vector2(1, 1)
const CAN_DROP_HOTSPOT := Vector2(1, 1)
const FORBIDDEN_HOTSPOT := Vector2(1, 1)
...
func apply_cursor_theme() -> void:
	...
	var drag_cursor := load(DRAG_CURSOR_PATH) as Texture2D
	var can_drop_cursor := load(CAN_DROP_CURSOR_PATH) as Texture2D
	var forbidden_cursor := load(FORBIDDEN_CURSOR_PATH) as Texture2D
	if (
		arrow_cursor == null
		or pointer_cursor == null
		or drag_cursor == null
		or can_drop_cursor == null
		or forbidden_cursor == null
	):
	...
	Input.set_custom_mouse_cursor(
		_scaled_cursor(drag_cursor),
		Input.CURSOR_DRAG,
		_scaled_hotspot(DRAG_HOTSPOT)
	)
	Input.set_custom_mouse_cursor(
		_scaled_cursor(drag_cursor),
		Input.CURSOR_MOVE,
	...
	Input.set_custom_mouse_cursor(
		_scaled_cursor(can_drop_cursor),
		Input.CURSOR_CAN_DROP,
		_scaled_hotspot(CAN_DROP_HOTSPOT)
	)
	Input.set_custom_mouse_cursor(
		_scaled_cursor(forbidden_cursor),
		Input.CURSOR_FORBIDDEN,
		_scaled_hotspot(FORBIDDEN_HOTSPOT)
	)
