...
var pending_npc_metadata_requests: Dictionary = {}
...
func get_npc_metadata(npc_id: String) -> Dictionary:
	...
	if pending_npc_metadata_requests.has(cache_key):
		var pending := pending_npc_metadata_requests[cache_key] as PendingRequest
		...
		return pending_result as Dictionary if pending_result is Dictionary else {
			"success": false,
			"error": "NPC metadata request completed without a result",
			"metadata": {},
		}

	var pending := PendingRequest.new()
	pending_npc_metadata_requests[cache_key] = pending
	var response := await _fetch_npc_metadata(normalized_npc_id, locale)
	var result: Dictionary = response
	if response.get("success", false):
		var normalized_metadata := _normalize_npc_metadata(
			normalized_npc_id,
	...
	pending_npc_metadata_requests.erase(cache_key)
