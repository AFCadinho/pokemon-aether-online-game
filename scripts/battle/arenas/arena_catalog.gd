...
const IDS := ["classic", "forest", "cave", "sea", "stadium", "route_1", "route_22", "route_22_water"]
...
static func uses_forest_assets(id: String) -> bool:
	return id in ["forest", "route_1", "route_22", "route_22_water"]
...
static func build(id: String, world: Node3D, camera: Camera3D = null) -> Node3D:
	if uses_forest_assets(id) and not Art.mounted_path.is_empty() and Art.error.is_empty():
		...
		if id == "route_1":
			return preload("res://scripts/battle/arenas/maps/route_1/arena.gd").new().build(camera)
