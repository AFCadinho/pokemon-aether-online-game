...
func _cliff_terraces(parent: Node3D) -> void:
	...
	for row in 2:
		...
		for i in 24:
			...
			rock.scale = Vector3(4.3, 3.15 if row == 0 else 2.45, 3.3) / bounds.size
			var ledge_y := BASE_HEIGHT - (0.1 if row == 0 else 2.4)
			rock.position = Vector3(x, ledge_y, z) - Vector3(
				bounds.get_center().x, bounds.position.y, bounds.get_center().z) * rock.scale
			_tint_rock(rock)
