...
const UTILITY_BAG_ACTIVE_BG := Color("#082955f2")
const UTILITY_BAG_ACTIVE_BG_HOVER := Color("#0b3970f6")
const UTILITY_BAG_ACTIVE_TEXT := Color("#e4f7ff")
...
@onready var buttons: Array[Button] = [run_button, bag_button]
...
func _apply_button_selected_state(button: Button) -> void:
	button.modulate = Color.WHITE
	button.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	var active_background := UTILITY_EXIT_ACTIVE_BG if button == run_button else UTILITY_BAG_ACTIVE_BG
	var active_hover_background := UTILITY_EXIT_ACTIVE_BG_HOVER if button == run_button else UTILITY_BAG_ACTIVE_BG_HOVER
	var active_text := UTILITY_EXIT_ACCENT if button == run_button else UTILITY_BAG_ACTIVE_TEXT
