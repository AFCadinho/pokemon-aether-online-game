...
const SiteMenu := preload("res://scripts/ui/cerulean_mountain_site_menu.gd")
...
@export var east_destination_position := Vector2(2192, 1776)
...
func interact_with_player(_player: Node2D) -> void:
	...
	if site_id.is_empty():
		return
	var destination := west_destination_position if site_id == WEST_SITE else east_destination_position
	var requirement := WEST_LEVEL if site_id == WEST_SITE else EAST_LEVEL
	if level < requirement:
		await show_dialogue([LocalizationManager.text("npc.cerulean_mountain_guide.level_locked", {"level": requirement})])
		return
	...
	if world == null or not world.has_method("relocate_player_within_current_map"):
		...
		return
	var result: Dictionary = await world.call("relocate_player_within_current_map", destination)
...
func can_access_site(site_id: String, rock_smash_level: int) -> bool:
	return rock_smash_level >= (EAST_LEVEL if site_id == EAST_SITE else WEST_LEVEL)
