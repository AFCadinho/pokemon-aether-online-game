...
func interact_with_player(_player: Node2D) -> void:
	...
	if not bool(metadata_response.get("success", false)):
		await show_dialogue([
			LocalizationManager.text("npc.aether_atelier.records_unavailable"),
			LocalizationManager.text("npc.error.try_again"),
		])
		return
	...
	if ui_overlay != null and ui_overlay.has_method("open_aether_atelier"):
		...
		return
	await show_dialogue([LocalizationManager.text("npc.aether_atelier.counter_unavailable")])
