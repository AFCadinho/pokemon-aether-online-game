...
func get_water_layer_count() -> int:
	return _water_layers.size()
...
func _draw_rain_impact(event: GroundEvent, progress: float) -> void:
	if event.surface_type == SURFACE_WATER:
		_draw_rain_ripple(event, progress)
		return
...
func _draw_rain_ripple(event: GroundEvent, progress: float) -> void:
	...
	if progress < 0.18:
		...
		draw_circle(event.world_position, 0.9 * event.size, contact_color)
...
func _spawn_event() -> GroundEvent:
	var event_surface := _find_event_surface()
	if event_surface.is_empty():
		return null
	var event := GroundEvent.new()
	event.world_position = event_surface.get("position", Vector2.ZERO)
	event.surface_type = str(event_surface.get("surface_type", SURFACE_GROUND))
...
func _find_event_surface() -> Dictionary:
	for _attempt in 18:
		var candidate := Vector2(
			_rng.randf_range(spawn_rect.position.x, spawn_rect.end.x),
			_rng.randf_range(spawn_rect.position.y, spawn_rect.end.y)
		)
		var surface_type := _weather_surface_type_at_position(candidate)
		if surface_type != "":
			return {"position": candidate, "surface_type": surface_type}
	return {}


func _is_position_on_weather_surface(world_position: Vector2) -> bool:
	return _weather_surface_type_at_position(world_position) != ""


func _weather_surface_type_at_position(world_position: Vector2) -> String:
	...
	if _surface_layers.is_empty() and _water_layers.is_empty():
		return SURFACE_GROUND if _cover_layers.is_empty() else ""
	if _has_tile_at_world_position(_cover_layers, world_position):
		return ""
	if _has_tile_at_world_position(_water_layers, world_position):
		return SURFACE_WATER if effect_type == EFFECT_RAIN else ""
	if _has_tile_at_world_position(_surface_layers, world_position):
