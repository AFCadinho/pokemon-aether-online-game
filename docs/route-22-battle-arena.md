...
battle) resolve to `route_22` in automatic 3D presentation. Water encounters,
PvP, and explicit environment/manual arena overrides retain their precedence.
The existing grass images remain the 2D presentation for this environment.
...
forest's battle presentation. Both fighter spawns are flat at `surface_height`.

## Preparation and ownership

`uses_forest_assets()` covers forest and Route 22, including asynchronous pack
...
The preview uses the client camera and light rig, with no placeholder Pokémon.
