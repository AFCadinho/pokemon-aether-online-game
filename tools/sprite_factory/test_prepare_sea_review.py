...
class SeaPreparationTests(unittest.TestCase):
    def test_prepare_and_refresh_only_review_files(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('sys.argv', ['prepare', str(project)]):
                prepare_sea_review.main()
                prepare_sea_review.main()
            self.assertTrue((project / '.pokeaether-sea-review').is_file())
            self.assertTrue((project / 'tools/sprite_factory/sea_battle_review.gd').is_file())
    ...
    def test_reject_frontend_checkout(self):
        source = Path(prepare_sea_review.__file__).resolve().parents[2]
